...
- Default workspace probing now prioritizes project-local `.Chonos/`, then falls back to `~/.Chonos/workspace`; default runtime artifacts are now under `<workspace>/config/` (for example `config.json`, `todo.db`).
