...
智能周期任务管理器。支持6种周期类型、每月N次配额、自动cron、统一视图。
...
python3 skills/chronos/scripts/unified_todo.py add "华夏10分精彩" \
  --financial \
...
# 盛京银行：每月2次，每周三10:00
python3 skills/chronos/scripts/unified_todo.py add "盛京银行抢携程500-50券" \
  --financial \
  --cycle-type monthly_n_times \
  --weekday 2 \
  --n-per-month 2 \
